...
| `uses_list` | Присутствует для любого объекта (если есть зависимости). Массив объектов, используемых данным объектом (см. ниже) |
...
| Поле | Описание |
|---|---|
| `schema_name` | Схема используемого объекта |
| `object_name` | Имя используемого объекта |
| `database_name` | База данных используемого объекта. Для MSSQL — текущая БД для локальных объектов, фактическое имя БД для межбазовых ссылок. Для Oracle — всегда пустая строка |
| `kind` | Тип используемого объекта: `TABLE`, `VIEW`, `PROCEDURE`, `FUNCTION`, `TRIGGER`, `SYNONYM`, `SEQUENCE`, `PACKAGE` (Oracle), `TYPE` (Oracle) и др. |
